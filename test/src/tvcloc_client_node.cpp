...
TvcLocClient::TvcLocClient():priv_node_("~")
{
  ...
  localization_service_ = priv_node_.advertiseService("control_localization", &TvcLocClient::triggerLocalization, this);
  if(msg_type_ == 0x01) {
...
void TvcLocClient::odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  else if (msg_type_== 0x01 && msg_type_version_ == 0x04)
  {
    ...
    int16_t omega = msg->twist.twist.angular.z / M_PI * 180 * 1000 ;
...
bool TvcLocClient::triggerLocalization(tvcloc_client::localizationRequest &req,
    tvcloc_client::localizationResponse &res)
{
  int8_t command = req.command;
  uint16_t dataArr[NUM_HOLDING_REGS_CMD];
  uint64_t timestamp = ros::Time::now().toNSec();
  dataArr[1] = (uint16_t)((timestamp & 0xFFFF000000000000) >> 48);
  dataArr[2] = (uint16_t)((timestamp & 0xFFFF00000000) >> 32);
  dataArr[3] = (uint16_t)((timestamp & 0xFFFF0000) >> 16);
  dataArr[4] = (uint16_t)(timestamp & 0xFFFF);
  dataArr[5] = (uint16_t)3 << 8 | (uint16_t)6; //type3, version 6
  dataArr[6] = 0;
  if(command == 0) {
    //stop localization
    dataArr[7] = 0;
    ...
    ROS_INFO("localization command not supported");
    res.succeed = false;
    return false;
  }
  if(!client_->beginTransmission(slave_id_, HOLDING_REGISTERS, startHoldingRegsAddr_, NUM_HOLDING_REGS_CMD)) {
    ROS_ERROR("Begin transmission fail!");
    res.succeed = false;
    return false;
  }
  for (int i = 0; i < NUM_HOLDING_REGS_CMD; i++){
    if(!client_->write(dataArr[i])) {
      ROS_ERROR("Write data fail");
      res.succeed = false;
      return false;
